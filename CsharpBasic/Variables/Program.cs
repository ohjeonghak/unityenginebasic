...
namespace Variables
{
    internal class Program
    {
        ...
        {
            // 자료형 변수이름
            int number1 = 5; // 부호가 있는 4byte 정수형 , 표현범위 : -2^31 ~ 2^31 - 1 (마이너스 표현 = 2의 보수 : 모든 비트 반전후 + 1 )
            ...
            long1 = number1; // 자료형의 승격
                             // ( 작은 레지스터 연산 결과를 큰 레지스터로 쓸떄는 자의 손실이 없으므로 데이터를 쓰는데 문제가 없다)
                             // 암시적 형변환(캐스팅)
            ...
            double double1 = 3.0;
            bool bool1 = true; // 1byte 논리형 (boolrean) 0일 경우 false, 0이 아닐경우 true
